...
int janus_process_incoming_request(janus_request *request)
{
	...
	if (session_id == 0 && handle_id == 0)
	{
		...
		if (janus_auth_is_enabled())
		{
			...
			json_t *token = json_object_get(root, "token");
			// ret = janus_auth_jwt_verify(json_string_value(token), user_id);
			// // ret = janus_request_check_secret(request, session_id, transaction_text);
			user_id = janus_auth_jwt_verify(json_string_value(token));
			if (!user_id)
			{
		...
		session->user_id = user_id != NULL ? strdup(user_id) : NULL;
		...
		json_object_set_new(data, "user_id", json_string(user_id));
