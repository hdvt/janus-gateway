...
typedef struct janus_rabbitmq_client {
	...
	amqp_bytes_t event_exchange;			/* AMQP exchange for outgoing messages */
	amqp_bytes_t event_queue;
	amqp_bytes_t request_queue;			/* AMQP outgoing messages queue (Janus API) */
	amqp_bytes_t response_queue;			/* AMQP incoming messages queue (Janus API) */
...
/* Global properties */
static char *node_id = NULL, *rmqhost = NULL, *vhost = NULL, *username = NULL, *password = NULL,
	*ssl_cacert_file = NULL, *ssl_cert_file = NULL, *ssl_key_file = NULL,
	*request_queue = NULL, *response_queue = NULL, *to_janus_admin = NULL, *from_janus_admin = NULL, *event_exchange = NULL, *event_queue = NULL, *janus_exchange = NULL, *janus_exchange_type = NULL;
...
int janus_rabbitmq_init(janus_transport_callbacks *callback, const char *config_path) {
	...
	if(!item || !item->value || !janus_is_true(item->value)) {
		...
		/* Parse configuration */
		item = janus_config_get(config, config_general, janus_config_type_item, "node_id");
		if(!item || !item->value) {
			JANUS_LOG(LOG_FATAL, "Missing name of node id for RabbitMQ integration...\n");
			goto error;
		}
		node_id = g_strdup(item->value);
		item = janus_config_get(config, config_general, janus_config_type_item, "request_queue");
		if(!item || !item->value) {
			JANUS_LOG(LOG_FATAL, "Missing name of request queue for RabbitMQ integration...\n");
			goto error;
		}
		request_queue = g_strdup(item->value);
		item = janus_config_get(config, config_general, janus_config_type_item, "response_queue");
		if(!item || !item->value) {
			JANUS_LOG(LOG_FATAL, "Missing name of response queue for RabbitMQ integration...\n");
			goto error;
		}
		response_queue = g_strdup(item->value);
		...
		if(!item || !item->value) {
			JANUS_LOG(LOG_INFO, "Missing name of event exchange for RabbitMQ integration, using default\n");
		} else {
			event_exchange = g_strdup(item->value);
		}
		item = janus_config_get(config, config_general, janus_config_type_item, "event_queue");
		if(!item || !item->value) {
			JANUS_LOG(LOG_FATAL, "Missing name of event queue for RabbitMQ integration...\n");
			goto error;
		}
		...
		// }
		JANUS_LOG(LOG_INFO, "RabbitMQ support for Janus API enabled, %s:%d (%s/%s/%s) exch: (%s) exchange_type:%s \n", rmqhost, rmqport, request_queue, response_queue, event_queue, event_exchange, janus_exchange_type);
	...
	if(!rmq_janus_api_enabled && !rmq_admin_api_enabled) {
		...
		if(rmq_janus_api_enabled) {
			...
			JANUS_LOG(LOG_VERB, "Declaring request queue... (%s)\n", request_queue);
			rmq_client->request_queue = amqp_cstring_bytes(request_queue);
			amqp_queue_declare(rmq_client->rmq_conn, rmq_client->rmq_channel, rmq_client->request_queue, 0, 0, 0, 1, amqp_empty_table);
			result = amqp_get_rpc_reply(rmq_client->rmq_conn);
			if(result.reply_type != AMQP_RESPONSE_NORMAL) {
				JANUS_LOG(LOG_FATAL, "Can't connect to RabbitMQ server: error declaring queue... %s, %s\n", amqp_error_string2(result.library_error), amqp_method_name(result.reply.id));
				goto error;
			}
			JANUS_LOG(LOG_VERB, "Declaring response queue... (%s)\n", response_queue);
			rmq_client->response_queue = amqp_cstring_bytes(response_queue);
			amqp_queue_declare(rmq_client->rmq_conn, rmq_client->rmq_channel, rmq_client->response_queue, 0, 0, 0, 0, amqp_empty_table);
			result = amqp_get_rpc_reply(rmq_client->rmq_conn);
			if(result.reply_type != AMQP_RESPONSE_NORMAL) {
				JANUS_LOG(LOG_FATAL, "Can't connect to RabbitMQ server: error declaring queue... %s, %s\n", amqp_error_string2(result.library_error), amqp_method_name(result.reply.id));
				goto error;
			...
			JANUS_LOG(LOG_VERB, "Declaring exchange...\n");
			rmq_client->event_exchange = amqp_cstring_bytes(event_exchange);
			amqp_exchange_declare(rmq_client->rmq_conn, rmq_client->rmq_channel, rmq_client->event_exchange, amqp_cstring_bytes(janus_exchange_type), 0, 0, 0, 0, amqp_empty_table);
			result = amqp_get_rpc_reply(rmq_client->rmq_conn);
			if(result.reply_type != AMQP_RESPONSE_NORMAL) {
				JANUS_LOG(LOG_FATAL, "Can't connect to RabbitMQ server: error diclaring exchange... %s, %s\n", amqp_error_string2(result.library_error), amqp_method_name(result.reply.id));
				goto error;
			}
			JANUS_LOG(LOG_VERB, "Declaring event queue... (%s)\n", event_queue);
			rmq_client->event_queue = amqp_cstring_bytes(event_queue);
			amqp_queue_declare(rmq_client->rmq_conn, rmq_client->rmq_channel, rmq_client->event_queue, 0, 0, 0, 0, amqp_empty_table);
			result = amqp_get_rpc_reply(rmq_client->rmq_conn);
			if(result.reply_type != AMQP_RESPONSE_NORMAL) {
				JANUS_LOG(LOG_FATAL, "Can't connect to RabbitMQ server: error declaring queue... %s, %s\n", amqp_error_string2(result.library_error), amqp_method_name(result.reply.id));
				goto error;
			}
			amqp_queue_bind(rmq_client->rmq_conn, rmq_client->rmq_channel, rmq_client->event_queue, rmq_client->event_exchange, amqp_cstring_bytes(""), amqp_empty_table);
			result = amqp_get_rpc_reply(rmq_client->rmq_conn);
			if(result.reply_type != AMQP_RESPONSE_NORMAL) {
				JANUS_LOG(LOG_FATAL, "RabbitMQEventHandler: Can't connect to RabbitMQ server: error bind rmq_event_queue... %s, %s\n", amqp_error_string2(result.library_error), amqp_method_name(result.reply.id));
				return -1;
			}
			amqp_basic_consume(rmq_client->rmq_conn, rmq_client->rmq_channel, rmq_client->request_queue, amqp_empty_bytes, 0, 1, 0, amqp_empty_table);
			result = amqp_get_rpc_reply(rmq_client->rmq_conn);
			if(result.reply_type != AMQP_RESPONSE_NORMAL) {
...
error:
	...
	g_free(username);
	g_free(password);
	g_free(event_exchange);
	g_free(request_queue);
	g_free(response_queue);
	g_free(event_queue);
	...
	while(!rmq_client->destroy && !g_atomic_int_get(&stopping)) {
		...
		if(!rmq_client->destroy && !g_atomic_int_get(&stopping) && response->payload) {
			...
			int status = amqp_basic_publish(rmq_client->rmq_conn, rmq_client->rmq_channel, amqp_empty_bytes,
				response->admin ? rmq_client->from_janus_admin_queue : rmq_client->response_queue,
				0, 0, &props, message);
			if(status != AMQP_STATUS_OK) {
